...
class database():
    ...
    def update_dataInput(self,check,date,code,g_type,g_thick,g_wide,g_long,quantity,volume,supplier):
        ...
        sql = 'UPDATE Wood SET Wood_code=%s , ' \
        ...
        value = (code,g_type,g_thick,g_wide,g_long,supplier,date,quantity,volume,check)
    ...
    def func_check_quantity_wood(self,listtable):
        conn = self.generate_conn_singleton()
        cur = conn.cursor()
        sql = "SELECT Quantity  " \
              "FROM stock " \
              "INNER JOIN Wood ON stock.wood_id = Wood.Wood_id " \
              "where Wood_code Like '%" + listtable + "%'"
        cur.execute(sql)
        data = cur.fetchone()
        return data
    ...
    def func_insert_withdrawcut_tosql(self,date,typewithdraw,woodcode):
        print(woodcode)
        conn = self.generate_conn_singleton()
        cur = conn.cursor()
        sql = """INSERT INTO withdraw (date,withdrawtype,woodid) VALUES (%s,%s,(SELECT wood_id FROM wood WHERE wood_id = (SELECT wood_id FROM wood WHERE Wood_code=%s ))) """
        value = (date,typewithdraw,woodcode)
